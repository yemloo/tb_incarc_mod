...
prison_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/argentina_prison_incid_231211.csv')))
who_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/argentina_whoincidfactor_231211.csv')))
all_samps <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/argentina_samps_240617.csv')))
...
idx_list <- gsub('argentina_240617_incarc_mod_fit_|.RDS', '', 
...
ggpairs(all_params_dt[incarc_optim_error <= quantile(all_params_dt$incarc_optim_error, 0.95),
                      c('recid_factor','admissions.factor','change.r.factor1',
                        'iR', 'iE:iR','k1','k2','k3','covidf','r','incarc_optim_error')],
        upper = list(continuous=wrap('density', alpha=0.5)),
        lower = list(continuous = wrap("points", alpha=0.5, size=0.5))) +
  theme_classic() +
  theme(axis.text.x=element_text(angle=45, hjust=1))
...
ggpairs(all_params_dt[tb_optim_error <= quantile(all_params_dt$tb_optim_error, 0.95),
                      c('beta_pp','beta_cc','c1','c4','v','z','d4','d1','p',
...
tb_optim_e3_thresh <- 0.25
tb_optim_e4_thresh <- 0.1
