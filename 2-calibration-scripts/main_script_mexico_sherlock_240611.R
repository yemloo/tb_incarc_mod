...
all_samps <- fread(paste0(wd_params, 'mexico_samps_240611.csv')) # change
who_incid_factor_samp <- as.matrix(fread(paste0(wd_params,'mexico_whoincidfactor_231211.csv')))
prison_incid_samp <- as.matrix(fread(paste0(wd_params,'mexico_prison_incid_231211.csv')))
...
all_samps_calib_start <- read.csv(paste0(wd_params, 'mex_samps_calib_start_240611.csv'), header=T, check.names = FALSE) # change

####### calibration targets ####### 
incarc_prev=latam_incarc_data[Country==country_p & !is.na(`Adjusted Incarceration Prevalence`),
                              `Adjusted Incarceration Prevalence`]
# Mexico & Colombia only: replace first value for incarc prev with something between observed value & splines fit
spline.prev.1990 <- smooth.spline(latam_incarc_data[Country==country_p & !is.na(`Adjusted Incarceration Prevalence`),
                                                  `Adjusted Incarceration Prevalence`], spar=0.6)$y[1]
incarc.prev.1990.weight <- all_samps[i,]$incarc.prev.1990.weight # change
incarc_prev[1] <- incarc.prev.1990.weight*incarc_prev[1]+(1-incarc.prev.1990.weight)*spline.prev.1990
...
who_incid_factor=who_incid_factor_samp[,i][-5] # Mexico-only: exclude 2004 as calibration target; outlier & throwing off fit
...
recid_odds=latam_incarc_data[Country==country_p & !is.na(`Readmissions percent`), # in Mexico recidivism target is the % of all admissions that have history of incarc
                             `Readmissions percent`] /
  (1-latam_incarc_data[Country==country_p & !is.na(`Readmissions percent`),
                       `Readmissions percent`]) * 
...
recid_yr=latam_incarc_data[Country==country_p & !is.na(`Readmissions percent`),
                           Year]
...
beta_pc=all_samps[i,]$beta_pc # change
b=all_samps[i,]$b
...
# c2.perc.change=-all_samps[i,]$c2.perc.change # note the negative sign here bc we want percent decrease 
c2.perc.change=0 # as of 6/6/24 not doing this anymore
v1.ratio=all_samps[i,]$v1.ratio
...
incarc_param_names <- c('iR','iE:iR','r','k1','k2','k3')
incarc_params_lower_bounds= c(0.01, 0.01, 0.5, 0.00001, -0.001, 0.00005)
incarc_params_upper_bounds= c(0.1, 0.2, 3, 0.001, -0.0001, 0.0002)
...
tb_param_names <- c('beta_pp',
                    ...
                    'd',
                    'covid.d.factor')
tb_params_lower_bounds = c(20, # beta_pp
                           0.3, # beta_cc:beta_pp
                           0.06, # c1
                           0.2, # c4:c1
                           0.1, # v (change in diagnosis rate outside prison)
                           1.0, # z (change in beta outside prison period 1),
                           -0.6, # ratio of z2 (period 2) to z
                           0.4, # ratio of z3 (period 3) to z
                           -2, # p (change in beta prison)
                           0.5, # baseline diagnosis rate outside prison
                           0.5) # relative diagnosis rate during COVID (d_covid = d*param)
tb_params_upper_bounds = c(40, # beta_pp
                           0.45, # beta_cc:beta_pp
                           0.16, # c1
                           0.8, # c4:c1
                           0.4, # v (change in diagnosis rate outside prison)
                           1.5, #  z (change in beta outside prison period 1)
                           -0.4, # ratio of z2 (period 2) to z
                           0.6, # ratio of z3 (period 3) to z
                           -0.2, # p (change in beta prison)
                           1.0, # baseline diagnosis rate
                           0.8) # relative diagnosis rate during COVID (d_covid = d*param)
