...
all_samps <- fread(paste0(wd_params, 'peru_samps_240612.csv'))
who_incid_factor_samp <- as.matrix(fread(paste0(wd_params,'peru_whoincidfactor_231211.csv')))
prison_incid_samp <- as.matrix(fread(paste0(wd_params,'peru_prison_incid_231211.csv')))
...
all_samps_calib_start <- read.csv(paste0(wd_params, 'peru_samps_calib_start_240612.csv'), header=T, check.names = FALSE)
...
incarc_params_lower_bounds=c(1e-02, 0.005, 0.75, 3e-05, -7e-05, -8e-05, 0.5)
incarc_params_upper_bounds=c(0.15, 0.1, 1.5, 1e-04, -3e-05, -4e-05, 0.85)
...
tb_params_lower_bounds = c(45, # beta_pp
                           0.15, # beta_cc:beta_pp
                           0.1, # c1
                           0.4, # c4:c1
                           0.03, # v (change in diagnosis rate outside prison)
                           -0.9, # z (change in beta outside prison period 1),
                           0.4, # baseline diagnosis rate
                           0.4) # relative diagnosis rate during COVID (d_covid = d*param)
tb_params_upper_bounds = c(100, # beta_pp
                           0.5, # beta_cc:beta_pp
                           0.2, # c1
                           0.75, # c4:c1
                           0.08, # v (change in diagnosis rate outside prison)
