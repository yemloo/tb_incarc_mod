...
all_samps <- fread(paste0(wd_params, 'colombia_samps_240612.csv'))
who_incid_factor_samp <- as.matrix(fread(paste0(wd_params,'colombia_whoincidfactor_231211.csv')))
prison_incid_samp <- as.matrix(fread(paste0(wd_params,'colombia_prison_incid_231211.csv')))
...
all_samps_calib_start <- read.csv(paste0(wd_params, 'colombia_samps_calib_start_240612.csv'), header=T, check.names = FALSE)
...
admissions=latam_incarc_data[Country==country_p & !is.na(`Admissions Rate`) & Year != 2022, # exclude 2022 bc we are not sure it's reliable
                             `Admissions Rate`] # admissions rate per 100k
admissions.years=latam_incarc_data[Country==country_p & !is.na(`Admissions Rate`) & Year != 2022, Year]
...
incarc_param_names <- c('iR','iE:iR','r','k1','k2','covidf')
incarc_params_lower_bounds=c(0.0001, 0.0001, 0.3, 0.00001, -0.0002, 0.25)
incarc_params_upper_bounds=c(0.1, 0.1, 1.2, 0.0001, -0.00001, 0.6)
...
tb_params_lower_bounds = c(10, # beta_pp
                           0.2, # beta_cc:beta_pp
                           0.05, # c1
                           0.2, # c4:c1
                           0.01, # v (change in diagnosis rate outside prison)
                           -0.12, # z (change in beta outside prison period 1),
                           0.4, # baseline diagnosis rate
                           0.4, # d1:d4
                           # 0, # ratio of change in d1 to change in d4
                           0.4) # relative diagnosis rate during COVID (d_covid = d*param)
tb_params_upper_bounds = c(40, # beta_pp
                           0.6, # beta_cc:beta_pp
                           0.16, # c1
                           0.7, # c4:c1
                           0.06, # v (change in diagnosis rate outside prison)
                           -0.01, # z (change in beta outside prison period 1),
                           1, # baseline diagnosis rate
                           1, # d1:d4
                           # 1, # ratio of change in d1 to change in d4
                           0.7) #relative diagnosis rate during COVID (d_covid = d*param)
