...
prison_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/mexico_prison_incid_231211.csv')))
who_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/mexico_whoincidfactor_231211.csv')))
all_samps <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/mexico_samps_240611.csv')))
...
idx_list <- gsub('mexico_240611_incarc_mod_fit_|.RDS', '', 
...
ggpairs(all_params_dt[incarc_optim_error <= quantile(all_params_dt$incarc_optim_error, 0.95),
                      c('recid_factor','admissions.factor','change.r.factor1','change.r.factor2',
                        'iR', 'iE:iR','k1','k2','k3','r','incarc_optim_error')],
        upper = list(continuous=wrap('density', alpha=0.5)),
        lower = list(continuous = wrap("points", alpha=0.5, size=0.5))) +
  theme_classic() +
  theme(axis.text.x=element_text(angle=45, hjust=1))
...
ggpairs(all_params_dt[tb_optim_error <= quantile(all_params_dt$tb_optim_error, 0.95),
                      c('beta_pp','beta_cc','c1','c4','v','z','z2:z','z3:z','d4','p',
...
incarc_optim_error_thresh <- Inf
incarc_optim_e1_thresh <- 0.002
incarc_optim_e2_thresh <- Inf
incarc_optim_e3_thresh <- 0.0025
incarc_optim_e4_thresh <- Inf
incarc_optim_e5_thresh <- Inf
...
ggplot(incarc_mod_fits_dt[variable == 'recid' & incarc_optim_e4 <= incarc_optim_e4_thresh]) +
  ...
  geom_point(data=latam_incarc_data[Country == country_p & !is.na(`Readmissions percent`)], aes(x=Year, y=`Readmissions percent`)) +
  theme_bw() + scale_color_gradient(low = 'grey', high = 'red')
...
prison_tmp[!is.na(D), I := apply(prison_incid_samp, 1, median)/100000]
...
tb_optim_e1_thresh <- 0.5
tb_optim_e2_thresh <- Inf
tb_optim_e3_thresh <- 0.35
tb_optim_e4_thresh <- 0.13
...
ggplot() +
  ...
  aes(x=time, y=rate_per100k, group=idx, color=tb_optim_e4), linewidth=0.5) +
...
# apply same thresholds for sensitivity analyses; only include those with good fits for main & sens analysis
params_sens1_dt <- data.table()
exclude <- c()
...
tb_e_hi_idx_sens1 <- params_sens1_dt[tb_optim_error > tb_optim_error_thresh |
                                       tb_optim_e1 > tb_optim_e1_thresh |
                                       tb_optim_e2 > tb_optim_e2_thresh |
                                       tb_optim_e3 > tb_optim_e3_thresh |
                                       tb_optim_e4 > tb_optim_e4_thresh, idx]
...
# save good idx
saveRDS(all_outputs_dt[analysis == 'no change in beta_pp' & !idx %in% c(incarc_e_hi_idx, tb_e_hi_idx_sens1)], 
        paste0(wd_save, '3-outputs/good_outputs_',gsub(" ", "", tolower(country_p)), '_', today, '_sens1_nochange_betapp.RDS'))
