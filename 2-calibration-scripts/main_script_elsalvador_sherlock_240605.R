...
all_samps <- fread(paste0(wd_params, 'elsal_samps_240605.csv'))
who_incid_factor_samp <- as.matrix(fread(paste0(wd_params,'elsal_whoincidfactor_231211.csv')))
prison_incid_samp <- as.matrix(fread(paste0(wd_params,'elsal_prison_incid_231211.csv')))
...
all_samps_calib_start <- read.csv(paste0(wd_params, 'elsal_samps_calib_start_240605.csv'), header=T, check.names = FALSE)
...
incarc_prev_yrs=latam_incarc_data[Country==country_p & !is.na(`Adjusted Incarceration Prevalence`),
                                  Year]
fit.end.idx=c(0,4) # fit to 2019 and 2023 equally
...
prison_incid_yr <- sort(prison_estimates[Country == country_p & 
                                           !is.na(NR_Obs_New), # for Brazil and Colombia, empirical estimates only go thru 2019
                                         Year])
...
SoEf_iR=all_samps[i,]$SoEf_iR # elsal only
SoE_r=all_samps[i,]$SoE_r # elsal only
...
spike.d1.factor=all_samps[i,]$spike.d1.factor # elsal only
...
incarc_param_names <- c('iR','iE:iR','r','k1','k2','k3','SoEf','SoEf2')
incarc_params_lower_bounds=c(0.005, 0.0001, 0.35, 0.00015, 0.00015, -0.0004, 20, 1.5)
incarc_params_upper_bounds=c(0.2, 0.05, 0.7, 0.0004, 0.0004, -0.00015, 60, 4)
incarc_params_start=as.numeric(all_samps_calib_start[i,incarc_param_names])
...
tb_params_lower_bounds = c(15, # beta_pp
                           ...
                           0.08, # c1
                           0.2, # c4:c1
                           0.06, # v (change in diagnosis rate outside prison)
                           -1.5, # z (change in beta outside prison period 1),
                           0.4, # baseline diagnosis rate
                           0.2, # d1:d4
                           0.1, # ratio of change in d1 to change in d4
                           0.6, # relative diagnosis rate during COVID (d_covid = d*param)
                           0.15) # p (change in beta prison)
tb_params_upper_bounds = c(40, # beta_pp
                           0.85, # beta_cc:beta_pp
                           0.16, # c1
                           0.7, # c4:c1
                           0.15, # v (change in diagnosis rate outside prison)
                           -0.5, # z (change in beta outside prison period 1),
                           1, # baseline diagnosis rate
                           1, # d1:d4
                           1, # ratio of change in d1 to change in d4
                           0.95, # relative diagnosis rate during COVID
                           0.42) # p (change in beta prison)
...
if (country_p == "Mexico"){ # in Mexico, "recid.percent" is the percent of all *admissions* that are re-admissions
  ...
  percent_readmissions <- mean(readmissions_obs/all_admissions_obs)
...
##############################
## El Salvador only: no SoE ##
output_noSoE <- ode(
  func=tb_model_for_calibration,
  y=xstart,
  times=timeunit,
  parms=c(all_params, all.settings),
  iR_func=incarc_functions_noSoE$iR_func,
  iE_func=incarc_functions_noSoE$iE_func,
  r_func=incarc_functions_noSoE$r_func,
  quasi_2000=2000
)

output_dt_noSoE <- data.table(output_noSoE)
output_dt_noSoE[,`:=`(N1=S1+E1+L1+I1+R1,
                               N2=S2+E2+L2+I2+R2,
                               N3=S3+E3+L3+I3+R3,
                               N4=S4+E4+L4+I4+R4)]

# ggplot() +
#   geom_line(data=output_dt_noSoE[time>=1990], aes(x=time, y=N1/(N1+N2+N3+N4)*100000)) +
#   geom_line(data=output_dt_stable[time>=1990], aes(x=time, y=N1/(N1+N2+N3+N4)*100000))

rates_noSoE <- get_annualized_rates(output_dt_noSoE, horiz.start = floor(all.settings$state.emerg.start-3), horiz.end = horiz.end, by.increment = time.increment)
rates_noSoE <- rbind(rates_noSoE, data.table(pop='Prison', variable='pop',
                                                               rate_per100k=output_dt_noSoE[time >= floor(all.settings$state.emerg.start-3) & time <= horiz.end, N1/(N1+N2+N3+N4)]*100000,
                                                               total=NA, time=seq(floor(all.settings$state.emerg.start-3), horiz.end, by=time.increment)),
                              data.table(pop='Post-Release', variable='pop',
                                         rate_per100k=output_dt_noSoE[time >= floor(all.settings$state.emerg.start-3) & time <= horiz.end, N2/(N1+N2+N3+N4)]*100000,
                                         total=NA, time=seq(floor(all.settings$state.emerg.start-3), horiz.end, by=time.increment)),
                              data.table(pop='Formerly Incarc', variable='pop',
                                         rate_per100k=output_dt_noSoE[time >= floor(all.settings$state.emerg.start-3) & time <= horiz.end, N3/(N1+N2+N3+N4)]*100000,
                                         total=NA, time=seq(floor(all.settings$state.emerg.start-3), horiz.end, by=time.increment)),fill=T)

print('noSoE...done')
rates_noSoE$scenario <- 'No SoE'
rates_all <- rbind(rates_all, rates_noSoE)
rates_all$idx <- i

# ggplot() +
#   geom_line(data=rates_noSoE[time>=2020 & variable == 'I'], aes(x=time, y=rate_per100k, color='no SoE')) +
#   geom_line(data=rates_stable[time>=2020 & variable == 'I'], aes(x=time, y=rate_per100k, color='observed')) +
#   facet_wrap(~pop, scales='free_y')
# ggplot() +
#   geom_line(data=rates_stable[time>=1990 & time <= 2025 & variable == 'I'], aes(x=time, y=total, color='observed')) +
#   facet_wrap(~pop, scales='free_y') + ylim(0, NA)
...
# # 2/12/24: not possible to get adequate fit for sens 1 for El Salvador; do not run
