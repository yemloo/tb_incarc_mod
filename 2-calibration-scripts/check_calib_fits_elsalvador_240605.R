...
all_samps <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/elsal_samps_240605.csv')))
...
idx_list <- gsub('elsalvador_240605_incarc_mod_fit_|.RDS', '', 
...
saveRDS(all_params, file=paste0(wd_save, '5-params/all_params_',gsub(' ', '', tolower(country_p)), '_', today, '.RDS'))
...
write.csv(all_params_dt, file=paste0(wd_save, '5-params/all_params_',gsub(' ', '', tolower(country_p)), '_', today, '.csv'),
          row.names = F)
...
incarc_mod_fits_dt <- merge(incarc_mod_fits_dt, all_params_dt[,c('idx','incarc_optim_error','incarc_optim_e1',
                                                              ...
                                                              with=F],
                            by='idx')
write.csv(incarc_mod_fits_dt, file=paste0(wd_save, '3-outputs/incarc_mod_fits_',gsub(' ', '', tolower(country_p)), '_', today, '.csv'),
          row.names = F)
...
ggpairs(all_params_dt[incarc_optim_error <= quantile(all_params_dt$incarc_optim_error, 0.95),
                      c('recid_factor','admissions.factor','change.r.factor1',
                        'iR', 'iE:iR','k1','k3','SoEf','SoEf2','r','incarc_optim_error')],
        upper = list(continuous=wrap('density', alpha=0.5)),
        lower = list(continuous = wrap("points", alpha=0.5, size=0.5))) +
  theme_classic() +
  theme(axis.text.x=element_text(angle=45, hjust=1))
...
ggpairs(all_params_dt[tb_optim_error <= quantile(all_params_dt$tb_optim_error, 0.95),
                      c('beta_pp','beta_cc','c1','c4','v','z','d1','d4','v1','p',
...
incarc_optim_e1_thresh <- 0.015
incarc_optim_e2_thresh <- Inf
incarc_optim_e3_thresh <- Inf
...
# not calibrating to TB deaths but will visually check model fit to TB deaths
...
tb_optim_e1_thresh <- 0.6
tb_optim_e2_thresh <- Inf
tb_optim_e3_thresh <- 0.4
tb_optim_e4_thresh <- 0.17
...
# params_sens1_dt <- data.table() # no sens1 for elsal: impossible to get good fits
exclude <- c()
...
# tb_e_hi_idx_sens1 <- params_sens1_dt[tb_optim_error > quantile(all_params_dt$tb_optim_error, 0.85) |
#                                        tb_optim_e4 > 0.14 |
#                                        tb_optim_e3 > 0.38, idx] # nearly all 
