...
# source('~/Desktop/Stanford/TBprisons_git/1-data-and-settings/brazil_main_settings_240603.R') # script with country settings for time-varying params # local
...
source(paste0(wd, 'scripts/brazil_main_settings_240603.R')) # script with country settings for time-varying params
...
all_samps <- fread(paste0(wd_params, 'brazil_samps_240603.csv'))
who_incid_factor_samp <- as.matrix(fread(paste0(wd_params,'brazil_whoincidfactor_231211.csv')))
prison_incid_samp <- as.matrix(fread(paste0(wd_params,'brazil_prison_incid_231211.csv')))
...
all_samps_calib_start <- read.csv(paste0(wd_params, 'brazil_samps_calib_start_240603.csv'), header=T, check.names = FALSE)
...
prison_incid_yr <- sort(prison_estimates[Country == country_p & 
                                           !is.na(NR_Obs_New) & Year <= 2019, # for Brazil and Colombia, empirical estimates only go thru 2019
                                         Year])
...
incarc_param_names <- c('iR','iE:iR','r','k1','k4','covidf')
incarc_params_lower_bounds=c(0.1, 0.00001, 0.3, 1e-05, -2e-04, 0.7)
incarc_params_upper_bounds=c(0.5, 0.01, 1.6, 1e-04, -1e-06, 0.95)
...
tb_params_lower_bounds <- c(35, # beta_pp
                            0.1, # beta_cc:beta_pp
                            0.09, # c1
                            0.15, # c2:c1
                            0.005, # v (change in diagnosis rate outside prison)
                            -0.5, # z (change in beta outside prison period 1),
                            0.5, # baseline diagnosis rate
                            0.6, # relative diagnosis rate during COVID (d_covid = d*param)
                            0.8) # p (change in beta prison)
tb_params_upper_bounds <- c(80, # beta_pp
                            0.3, # beta_cc:beta_pp
                            0.16, # c1
                            0.75, # c2:c1
                            0.05, # v (change in diagnosis rate outside prison)
                            -0.1, #  z (change in beta outside prison period 1)
                            1.2, # baseline diagnosis rate
...
incarc_optim_res <- optim(par=incarc_params_start, fn=optim_for_iE_iR_perct_grow, 
                          ...
                          control=list(trace=T, parscale=abs(incarc_param_means),
...
saveRDS(incarc_mod_fit, paste0(wd, 'results/', tolower(country_p), '_', today, '/', 
                          tolower(country_p), '_', today, '_incarc_mod_fit_', i, '.RDS'))

# Uncomment below if you need to check fit during troubleshooting
# ggplot(data.table(time=incarc_prev_yrs, known=incarc_prev, obs=incarc_prev_obs),
#        aes(x=time, y=incarc_prev_obs)) +
#   geom_line(color='grey') + geom_point(aes(x=time, y=incarc_prev)) + theme_bw()
...
saveRDS(optim_res, paste0(wd, 'results/', tolower(country_p), '_', today, '/', 
                          tolower(country_p), '_', today, '_params_', i, '.RDS'))
...
if (all.settings[['change.betap.start1']] != Inf){
  ...
  output_counterfactual_sens1 <- ode(
    ...
    y=xstart,
    times=timeunit,
    parms=c(all_params_sens1, all.settings.sens1), # counterfct settings are the same as in sens1
    iR_func=get_iR_cf,
    iE_func=get_iE_cf,
    r_func=get_r_cf,
    quasi_2000=2000
  ...
  # write.csv(rates_all_sens1, paste0(wd, 'results/model_outputs_', tolower(country_p), '_', today, '/', 
  #                             tolower(country_p), '_', today, '_sens1_outputs_', i, '.csv'), row.names=F)
...
write.csv(rates_all_combined, paste0(wd, 'results/model_outputs_', tolower(country_p), '_', today, '/', 
                                  tolower(country_p), '_', today, '_all_outputs_', i, '.csv'), row.names=F)
