...
prison_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/peru_prison_incid_231211.csv')))
who_incid_samp <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/peru_whoincidfactor_231211.csv')))
all_samps <- as.matrix(fread(paste0(wd, 'params/uncertainty_analysis/peru_samps_240612.csv')))
...
idx_list <- gsub('peru_240612_incarc_mod_fit_|.RDS', '', 
...
ggpairs(all_params_dt[tb_optim_error <= quantile(all_params_dt$tb_optim_error, 0.95),
                      c('beta_pp','beta_cc','c1','c4','v','z','d4',
...
ggplot() +
  ...
  aes(x=time, y=rate_per100k, group=idx, color=tb_optim_e2), linewidth=0.5) +
...
# params_sens1_dt <- data.table() # no sens1 for peru
exclude <- c()
